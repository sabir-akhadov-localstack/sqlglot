...
def _build_date(args: t.List) -> exp.TsOrDsToDate:
...
def _build_month(args: t.List) -> exp.Month:
    return exp.Month(this=exp.TsOrDsToDate(this=seq_get(args, 0)))
...
def _build_to_days(args: t.List) -> exp.Paren:
    return exp.paren(
        exp.DateDiff(
            this=exp.TsOrDsToDate(this=seq_get(args, 0)),
...
def _build_week(args: t.List) -> exp.Week:
    return exp.Week(
...
def _build_year(args: t.List) -> exp.Year:
    return exp.Year(this=exp.TsOrDsToDate(this=seq_get(args, 0)))


def _parse_sounds_like(self: Parser, this: exp.Expr) -> exp.EQ:
...
def _parse_member_of(self: Parser, this: exp.Expr) -> exp.JSONArrayContains:
    return self.expression(
...
def _parse_group_concat_parser(self: Parser) -> exp.Expr:
    return self._parse_group_concat()  # type: ignore


def _parse_values_parser(self: Parser) -> exp.Anonymous:
    return self.expression(
        exp.Anonymous, this="VALUES", expressions=[self._parse_id_var()]
    )


def _parse_json_value_parser(self: Parser) -> exp.Expr:
    return self._parse_json_value()


def _parse_substr_parser(self: Parser) -> exp.Expr:
    return self._parse_substring()


def _show_statement(self: Parser) -> exp.Show:
    return self._parse_show()  # type: ignore


def _parse_lock_property(self: Parser) -> exp.Expr:
    return self._parse_property_assignment(exp.LockProperty)


def _parse_partition_by_property(self: Parser) -> t.Optional[exp.Expr] | t.List[exp.Expr]:
    return self._parse_partition_property()


def _parse_set_persist(self: Parser) -> exp.Expr:
    return self._parse_set_item_assignment("PERSIST")  # type: ignore


def _parse_set_persist_only(self: Parser) -> exp.Expr:
    return self._parse_set_item_assignment("PERSIST_ONLY")  # type: ignore


def _parse_set_character_set(self: Parser) -> exp.Expr:
    return self._parse_set_item_charset("CHARACTER SET")


def _parse_set_names(self: Parser) -> exp.Expr:
    return self._parse_set_item_names()


def _parse_constraint_fulltext(self: Parser) -> exp.IndexColumnConstraint:
    return self._parse_index_constraint(kind="FULLTEXT")


def _parse_constraint_index(self: Parser) -> exp.IndexColumnConstraint:
    return self._parse_index_constraint()


def _parse_constraint_spatial(self: Parser) -> exp.IndexColumnConstraint:
    return self._parse_index_constraint(kind="SPATIAL")


def _parse_constraint_zerofill(self: Parser) -> exp.ZeroFillColumnConstraint:
    return self.expression(exp.ZeroFillColumnConstraint)


def _parse_alter_modify(self: Parser) -> exp.Expr:
    return self._parse_alter_table_alter()  # type: ignore


def _parse_alter_alter_index(self: Parser) -> exp.AlterIndex:
    return self._parse_alter_table_alter_index()
...
_MYSQL_FUNCTION_PARSERS = {
    **_FUNCTION_PARSERS,
    "GROUP_CONCAT": _parse_group_concat_parser,
    # https://dev.mysql.com/doc/refman/5.7/en/miscellaneous-functions.html#function_values
    "VALUES": _parse_values_parser,
    "JSON_VALUE": _parse_json_value_parser,
    "SUBSTR": _parse_substr_parser,
}
...
_MYSQL_PROPERTY_PARSERS: t.Dict[str, t.Callable] = {
    **_PROPERTY_PARSERS,
    "LOCK": _parse_lock_property,
    "PARTITION BY": _parse_partition_by_property,
}

_MYSQL_ALTER_PARSERS = {
    **_ALTER_PARSERS,
    "MODIFY": _parse_alter_modify,
}
...
class Parser(_Parser):
    ...
    TABLE_ALIAS_TOKENS = (
        _TABLE_ALIAS_TOKENS - _TABLE_INDEX_HINT_TOKENS
    )

    RANGE_PARSERS = {
        **_RANGE_PARSERS,
        TokenType.SOUNDS_LIKE: _parse_sounds_like,
        TokenType.MEMBER_OF: _parse_member_of,
    }
    ...
    SET_PARSERS = {
        **_SET_PARSERS,
        "PERSIST": _parse_set_persist,
        "PERSIST_ONLY": _parse_set_persist_only,
        "CHARACTER SET": _parse_set_character_set,
        "CHARSET": _parse_set_character_set,
        "NAMES": _parse_set_names,
    }

    CONSTRAINT_PARSERS = {
        **_CONSTRAINT_PARSERS,
        "FULLTEXT": _parse_constraint_fulltext,
        "INDEX": _parse_constraint_index,
        "KEY": _parse_constraint_index,
        "SPATIAL": _parse_constraint_spatial,
        "ZEROFILL": _parse_constraint_zerofill,
    }
    ...
    ALTER_ALTER_PARSERS = {
        **_ALTER_ALTER_PARSERS,
        "INDEX": _parse_alter_alter_index,
    }
    ...
    PROFILE_TYPES: t.Dict[str, t.Sequence[t.Union[t.Sequence[str], str]]] = {
