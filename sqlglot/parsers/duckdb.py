...
from sqlglot.parser import Parser as _Parser, binary_range_parser, _FUNCTIONS, _FUNCTION_PARSERS, _NO_PAREN_FUNCTION_PARSERS, _RANGE_PARSERS, _BITWISE, _EXPONENT, _TABLE_ALIAS_TOKENS, _PLACEHOLDER_PARSERS, _STATEMENT_PARSERS, _SET_PARSERS, _FUNCTIONS_WITH_ALIASED_ARGS
from sqlglot.tokens import TokenType


def _build_sort_array_desc(args: t.List) -> exp.SortArray:
...
def _build_make_timestamp(args: t.List) -> exp.Func:
    if len(args) == 1:
...
def _build_any_value(args: t.List) -> exp.IgnoreNulls:
    return exp.IgnoreNulls(this=exp.AnyValue.from_arg_list(args))


def _build_array_intersect(args: t.List) -> exp.ArrayIntersect:
    return exp.ArrayIntersect(expressions=args)


def _build_decode(args: t.List) -> exp.Decode:
    return exp.Decode(this=seq_get(args, 0), charset=exp.Literal.string("utf-8"))


def _build_encode(args: t.List) -> exp.Encode:
    return exp.Encode(this=seq_get(args, 0), charset=exp.Literal.string("utf-8"))


def _build_epoch_ms(args: t.List) -> exp.UnixToTime:
    return exp.UnixToTime(this=seq_get(args, 0), scale=exp.UnixToTime.MILLIS)
...
def _build_list_value(args: t.List) -> exp.Array:
...
def _parse_string_agg(self: Parser) -> t.Optional[exp.Expr]:
    return self._parse_string_agg()


def _no_paren_map(self: Parser) -> exp.Expr:
    return self._parse_map()


def _no_paren_abs(self: Parser) -> exp.Abs:
    return exp.Abs(this=self._parse_bitwise())


def _placeholder_parameter(self: Parser) -> t.Optional[exp.Placeholder]:
...
def _build_text_type(dtype: exp.DataType) -> exp.DataType:
    return exp.DataType.build("TEXT")


def _attach_statement(self: Parser) -> exp.Expr:
    return self._parse_attach_detach()


def _detach_statement(self: Parser) -> exp.Expr:
    return self._parse_attach_detach(is_attach=False)


def _force_statement(self: Parser) -> exp.Expr:
    return self._parse_force()


def _install_statement(self: Parser) -> exp.Install:
    return self._parse_install()


def _show_statement(self: Parser) -> exp.Expr:
    return self._parse_show()  # type: ignore


def _set_variable_parser(self: Parser) -> t.Optional[exp.Expr]:
    return self._parse_set_item_assignment("VARIABLE")


class Parser(_Parser):
    MAP_KEYS_ARE_ARBITRARY_EXPRESSIONS = True

    BITWISE = _BITWISE.copy()
    ...
    FUNCTIONS_WITH_ALIASED_ARGS = {*_FUNCTIONS_WITH_ALIASED_ARGS, "STRUCT_PACK"}
    ...
    FUNCTIONS = {
        **_FUNCTIONS,
        ...
        "ENCODE": _build_encode,
        "EPOCH": exp.TimeToUnix.from_arg_list,
        "EPOCH_MS": _build_epoch_ms,
    ...
    FUNCTIONS = {k: v for k, v in FUNCTIONS.items() if k not in ("DATE_SUB", "GLOB")}

    FUNCTION_PARSERS = {
        **{k: v for k, v in _FUNCTION_PARSERS.items() if k != "DECODE"},
        **dict.fromkeys(
            ("GROUP_CONCAT", "LISTAGG", "STRINGAGG"), _parse_string_agg
        ),
    }
    ...
    PLACEHOLDER_PARSERS = {
        **_PLACEHOLDER_PARSERS,
        TokenType.PARAMETER: _placeholder_parameter,
    }
    ...
    TYPE_CONVERTERS = {
        ...
        exp.DType.TEXT: _build_text_type,
    }
    ...
    STATEMENT_PARSERS = {
        **_STATEMENT_PARSERS,
        ...
        TokenType.FORCE: _force_statement,
        TokenType.INSTALL: _install_statement,
        TokenType.SHOW: _show_statement,
    }

    SET_PARSERS = {
        **_SET_PARSERS,
        "VARIABLE": _set_variable_parser,
    }
