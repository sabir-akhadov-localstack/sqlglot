...
DATE_FMT_RE = __import__("re").compile(
    "([dD]{1,2})|([mM]{1,2})|([yY]{1,4})|([hH]{1,2})|([sS]{1,2})"
)
...
def _build_json_query(args: t.List, dialect: t.Any) -> exp.JSONExtract:
    if len(args) == 1:
...
def _build_charindex(args: t.List) -> exp.StrPosition:
...
def _build_count(args: t.List) -> exp.Count:
    return exp.Count(this=seq_get(args, 0), expressions=args[1:], big_int=False)


def _build_count_big(args: t.List) -> exp.Count:
    return exp.Count(this=seq_get(args, 0), expressions=args[1:], big_int=True)


def _build_isnull(args: t.List) -> exp.Coalesce:
    return build_coalesce(args=args, is_null=True)
...
def _option_modifier(self: _Parser) -> t.Tuple[str, t.Optional[t.List[exp.Expr]]]:
    return ("options", self._parse_options())  # type: ignore


def _for_modifier(self: _Parser) -> t.Tuple[str, t.Optional[t.List[exp.Expr]]]:
    return ("for_", self._parse_for())  # type: ignore


def _declare_statement(self: _Parser) -> exp.Declare:
    return self._parse_declare()  # type: ignore


def _execute_statement(self: _Parser) -> exp.Execute:
    return self._parse_execute()  # type: ignore


def _dcolon_range(self: _Parser, this: exp.Expr) -> exp.ScopeResolution:
...
def _json_arrayagg_parser(self: _Parser) -> exp.JSONArrayAgg:
    return self.expression(
        exp.JSONArrayAgg,
...
def _datepart_parser(self: _Parser) -> exp.Extract:
    return self._parse_datepart()  # type: ignore


def _dcolon_column_op(
    self: _Parser, this: exp.Expr, to: exp.Expr
) -> exp.Cast | exp.ScopeResolution:
    if isinstance(to, exp.DataType) and to.this != exp.DType.USERDEFINED:
        return self.expression(exp.Cast, this=this, to=to)
    return self.expression(exp.ScopeResolution, this=this, expression=to)
...
class Parser(_Parser):
    ...
    QUERY_MODIFIER_PARSERS = {
        **_QUERY_MODIFIER_PARSERS,
        TokenType.OPTION: _option_modifier,
        TokenType.FOR: _for_modifier,
    }

    # T-SQL does not allow BEGIN to be used as an identifier
    ID_VAR_TOKENS = _ID_VAR_TOKENS - {TokenType.BEGIN}
    ALIAS_TOKENS = _ALIAS_TOKENS - {TokenType.BEGIN}
    TABLE_ALIAS_TOKENS = _TABLE_ALIAS_TOKENS - {TokenType.BEGIN}
    COMMENT_TABLE_ALIAS_TOKENS = _COMMENT_TABLE_ALIAS_TOKENS - {TokenType.BEGIN}
    UPDATE_ALIAS_TOKENS = _UPDATE_ALIAS_TOKENS - {TokenType.BEGIN}

    FUNCTIONS = {
        **_FUNCTIONS,
        "ATN2": exp.Atan2.from_arg_list,
        "CHARINDEX": _build_charindex,
        "COUNT": _build_count,
        "COUNT_BIG": _build_count_big,
    ...
    RETURNS_TABLE_TOKENS = _ID_VAR_TOKENS - {
        TokenType.TABLE,
        *_TYPE_TOKENS,
    }
    ...
    STATEMENT_PARSERS = {
        **_STATEMENT_PARSERS,
        ...
        TokenType.EXECUTE: _execute_statement,
    }

    RANGE_PARSERS = {
        **_RANGE_PARSERS,
        TokenType.DCOLON: _dcolon_range,
    }
