...
from sqlglot.parser import _NO_PAREN_FUNCTIONS
from sqlglot.parsers.mysql import Parser as _MySQLParser, _MYSQL_FUNCTIONS, _MYSQL_FUNCTION_PARSERS, _MYSQL_PROPERTY_PARSERS
...
def _property_properties_parser(self: "Parser") -> t.List[exp.Property]:
    return self._parse_wrapped_properties()  # type: ignore


def _property_unique_parser(self: "Parser") -> exp.UniqueKeyProperty:
    return self._parse_composite_key_property(exp.UniqueKeyProperty)


def _property_key_parser(self: "Parser") -> exp.UniqueKeyProperty:
    return self._parse_composite_key_property(exp.UniqueKeyProperty)


def _property_build_parser(self: "Parser") -> exp.BuildProperty:
    return self._parse_build_property()


def _property_refresh_parser(self: "Parser") -> exp.RefreshTriggerProperty:
    return self._parse_refresh_property()
...
class Parser(_MySQLParser):
    ...
    FUNCTION_PARSERS = {k: v for k, v in _MYSQL_FUNCTION_PARSERS.items() if k != "GROUP_CONCAT"}

    NO_PAREN_FUNCTIONS = {k: v for k, v in _NO_PAREN_FUNCTIONS.items() if k != TokenType.CURRENT_DATE}

    PROPERTY_PARSERS = {
        **_MYSQL_PROPERTY_PARSERS,
        "PROPERTIES": _property_properties_parser,
        "UNIQUE": _property_unique_parser,
        "KEY": _property_key_parser,
        "BUILD": _property_build_parser,
        "REFRESH": _property_refresh_parser,
    }
