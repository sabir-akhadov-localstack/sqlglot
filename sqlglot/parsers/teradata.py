...
def _database_statement(self: "Parser") -> exp.Use:
    return self.expression(
        exp.Use, this=self._parse_table(schema=False)
    )


def _replace_statement(self: "Parser") -> exp.Create | exp.Command:
    return self._parse_create()


def _lock_statement(self: "Parser") -> exp.LockingStatement:
    return self._parse_locking_statement()


def _set_query_band_parser(self: "Parser") -> exp.QueryBand:
    return self._parse_query_band()


def _range_n_parser(self: "Parser") -> exp.RangeN:
    return self._parse_rangen()


def _translate_parser(self: "Parser") -> exp.TranslateCharacters:
    return self._parse_translate()


def _build_random(args: t.List) -> exp.Rand:
    return exp.Rand(lower=seq_get(args, 0), upper=seq_get(args, 1))
...
class Parser(_Parser):
    ...
    STATEMENT_PARSERS = {
        **_STATEMENT_PARSERS,
        TokenType.DATABASE: _database_statement,
        TokenType.REPLACE: _replace_statement,
        TokenType.LOCK: _lock_statement,
    }
    ...
    SET_PARSERS = {
        **_SET_PARSERS,
        "QUERY_BAND": _set_query_band_parser,
    }

    FUNCTION_PARSERS = {
        **_FUNCTION_PARSERS,
        # https://docs.teradata.com/r/SQL-Functions-Operators-Exprs-and-Predicates/June-2017/Data-Type-Conversions/TRYCAST
        "TRYCAST": _FUNCTION_PARSERS["TRY_CAST"],
        "RANGE_N": _range_n_parser,
        "TRANSLATE": _translate_parser,
    }
