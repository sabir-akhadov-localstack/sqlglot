...
from sqlglot.parser import Parser as _Parser, _FUNCTIONS, _FUNCTION_PARSERS, _PROPERTY_PARSERS, _NO_PAREN_FUNCTION_PARSERS, _NO_PAREN_FUNCTIONS, _RANGE_PARSERS, _COLUMN_OPERATORS, _JOIN_KINDS, _TABLE_ALIAS_TOKENS, _ALIAS_TOKENS, _FUNC_TOKENS, _RESERVED_TOKENS, _QUERY_MODIFIER_PARSERS, _CONSTRAINT_PARSERS, _ALTER_PARSERS, _SCHEMA_UNNAMED_CONSTRAINTS, _PLACEHOLDER_PARSERS, _ID_VAR_TOKENS
...
def _build_timestamp_trunc(unit: str) -> t.Callable[[t.List], exp.TimestampTrunc]:
    def _builder(args: t.List) -> exp.TimestampTrunc:
...
def _build_xor(args: t.List) -> exp.Xor:
    return exp.Xor(expressions=args)
...
def _build_agg_func_mapping(
    functions: t.Set[str], suffixes: t.List[str]
) -> t.Mapping[str, t.Tuple[str, str | None]]:
    result: t.Dict[str, t.Tuple[str, str | None]] = {
        f"{f}{sfx}": (f, sfx) for sfx in suffixes for f in functions
    }
    result.update({f: (f, None) for f in functions})
    return result


def _arrayjoin_parser(self: _Parser) -> exp.Explode:
    return self.expression(exp.Explode, this=self._parse_expression())


def _quantile_parser(self: _Parser) -> exp.Quantile:
    return self._parse_quantile()  # type: ignore


def _median_parser(self: _Parser) -> exp.Quantile:
    return self._parse_quantile()  # type: ignore


def _columns_parser(self: _Parser) -> exp.Expr:
    return self._parse_columns()  # type: ignore


def _tuple_parser(self: _Parser) -> exp.Struct:
    return exp.Struct.from_arg_list(self._parse_function_args(alias=True))


def _and_parser(self: _Parser) -> exp.And:
    return exp.and_(*self._parse_function_args(alias=False))  # type: ignore


def _or_parser(self: _Parser) -> exp.Or:
    return exp.or_(*self._parse_function_args(alias=False))  # type: ignore


def _engine_property_parser(self: _Parser) -> exp.EngineProperty:
    return self._parse_engine_property()  # type: ignore


def _global_range_parser(self: _Parser, this: t.Optional[exp.Expr]) -> exp.Not | exp.In:
    return self._parse_global_in(this)  # type: ignore


def _dotcaret_column_operator(
    self: _Parser, this: t.Optional[exp.Expr], field: exp.Expr
) -> exp.NestedJSONSelect:
    return self.expression(exp.NestedJSONSelect, this=this, expression=field)


def _settings_query_modifier(self: _Parser) -> t.Tuple[str, t.Any]:
    return ("settings", self._advance() or self._parse_csv(self._parse_assignment))


def _format_query_modifier(self: _Parser) -> t.Tuple[str, t.Any]:
    return ("format", self._advance() or self._parse_id_var())


def _index_constraint_parser(self: _Parser) -> exp.IndexColumnConstraint:
    return self._parse_index_constraint()  # type: ignore


def _codec_constraint_parser(self: _Parser) -> exp.Expr:
    return self._parse_compress()


def _assume_constraint_parser(self: _Parser) -> t.Optional[exp.AssumeColumnConstraint]:
    return self._parse_assume_constraint()  # type: ignore


def _modify_alter_parser(self: _Parser) -> t.Optional[exp.Expr]:
    return self._parse_alter_table_modify()  # type: ignore


def _replace_alter_parser(self: _Parser) -> t.Optional[exp.Expr]:
    return self._parse_alter_table_replace()  # type: ignore


def _l_brace_placeholder_parser(self: _Parser) -> t.Optional[exp.Expr]:
    return self._parse_query_parameter()  # type: ignore
...
class Parser(_Parser):
    ...
    FUNCTIONS = {k: v for k, v in FUNCTIONS.items() if k not in ("TRANSFORM", "APPROX_TOP_SUM")}
    ...
    RESERVED_TOKENS = _RESERVED_TOKENS - {TokenType.SELECT}
    ...
    AGG_FUNC_MAPPING: t.Mapping[str, t.Tuple[str, str | None]] = _build_agg_func_mapping(
    ...
    FUNCTION_PARSERS = {
        **{k: v for k, v in _FUNCTION_PARSERS.items() if k != "MATCH"},
        "ARRAYJOIN": _arrayjoin_parser,
        "QUANTILE": _quantile_parser,
        "MEDIAN": _median_parser,
        "COLUMNS": _columns_parser,
        "TUPLE": _tuple_parser,
        "AND": _and_parser,
        "OR": _or_parser,
    }
    ...
    PROPERTY_PARSERS = {
        **{k: v for k, v in _PROPERTY_PARSERS.items() if k != "DYNAMIC"},
        "ENGINE": _engine_property_parser,
    }

    NO_PAREN_FUNCTION_PARSERS = {k: v for k, v in _NO_PAREN_FUNCTION_PARSERS.items() if k != "ANY"}

    NO_PAREN_FUNCTIONS = _NO_PAREN_FUNCTIONS.copy()
    NO_PAREN_FUNCTIONS.pop(TokenType.CURRENT_TIMESTAMP)

    RANGE_PARSERS = {
        **_RANGE_PARSERS,
        TokenType.GLOBAL: _global_range_parser,
    }

    COLUMN_OPERATORS = {
        **_COLUMN_OPERATORS,
        TokenType.DOTCARET: _dotcaret_column_operator,
    }
    ...
    QUERY_MODIFIER_PARSERS = {
        **_QUERY_MODIFIER_PARSERS,
        TokenType.SETTINGS: _settings_query_modifier,
        TokenType.FORMAT: _format_query_modifier,
    }

    CONSTRAINT_PARSERS = {
        **_CONSTRAINT_PARSERS,
        "INDEX": _index_constraint_parser,
        "CODEC": _codec_constraint_parser,
        "ASSUME": _assume_constraint_parser,
    }

    ALTER_PARSERS = {
        **_ALTER_PARSERS,
        "MODIFY": _modify_alter_parser,
        "REPLACE": _replace_alter_parser,
    }
    ...
    PLACEHOLDER_PARSERS = {
        **_PLACEHOLDER_PARSERS,
        TokenType.L_BRACE: _l_brace_placeholder_parser,
    }

    def _parse_wrapped_select_or_assignment(self) -> t.Optional[exp.Expression]:
        def _parse_select_or_assignment() -> t.Optional[exp.Expression]:
            return self._parse_select() or self._parse_assignment()  # type: ignore

        return self._parse_wrapped(_parse_select_or_assignment, optional=True)  # type: ignore
